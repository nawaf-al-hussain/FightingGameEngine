...
static void loadMugenTruetypeFont(MugenDefScript* tScript, MugenFont* tFont) {
        ...
        freeMemory(name);
        if (!e.mFont) {
                // Font loading failed — remove this font from the map so
                // getUsedMugenFontFromAvailable doesn't return a broken entry
                logWarning("TrueType font loading failed. Font will not be available.");
                return;
        }
        tFont->mData = e;
}
...
static void addMugenFont2(int tKey, const char* tPath) {
        ...
        if (e.mType == MUGEN_FONT_TYPE_BITMAP) {
                ...
                // Check if font loading failed — if the truetype font wasn't set,
                // remove this entry so getUsedMugenFontFromAvailable skips it
                auto* truetypeData = std::get_if<MugenTruetypeFont>(&e.mData);
                if (!truetypeData || !truetypeData->mFont) {
