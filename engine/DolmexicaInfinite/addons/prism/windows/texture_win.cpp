...
#pragma warning( pop )
        ...
        {
                // Check if file exists (isFile calls getFullPath internally)
                if (!isFile(tName)) {
                        const auto systemPath = getSystemFontFile(tName);
                        if (!isFile(systemPath.c_str())) {
                                logWarningFormat("Font file not found: %s", tName);
                                return NULL;
                        }
                        // Use system path
                        tName = systemPath.c_str();
                }

                // Read font file into memory using engine's file I/O.
                // Pass tName directly — fileToBuffer/fileOpen calls getFullPath
                // internally. Do NOT pre-resolve the path with getFullPath, or
                // it will be double-resolved (producing ./font/./font/...).
                Buffer fontBuffer = fileToBuffer(tName);
                if (!fontBuffer.mData || fontBuffer.mLength == 0) {
                        logWarningFormat("Failed to read font file: %s", tName);
                        return NULL;
                }

                // Create SDL_RWops from memory buffer for SDL_ttf
                SDL_RWops* rw = SDL_RWFromMem(fontBuffer.mData, (int)fontBuffer.mLength);
                if (!rw) {
                        logWarningFormat("SDL_RWFromMem failed for font: %s", SDL_GetError());
                        freeBuffer(fontBuffer);
                        return NULL;
                }

                // TTF_OpenFontRW(rw, 1) — the '1' means SDL_ttf will free the RWops
                TTF_Font* font = TTF_OpenFontRW(rw, 1, (int)tSize);
                if (!font) {
                        logWarningFormat("TTF_OpenFontRW failed: %s (size=%d, buflen=%d)",
                                         TTF_GetError(), (int)tSize, (int)fontBuffer.mLength);
                }

                // Note: buffer memory is intentionally leaked — SDL_ttf/FreeType
                // may reference it. Fonts are loaded once and live for the session.
                return font;
        }
