...
static void loadFightDefFilesFromScript(MugenDefScript* tScript, char* tDefPath) {
        ...
        // FIX: Skip sprite/sound file loads when value is "NO_FILE" or missing.
        // The engine's recoverFromError() triggers screen transitions on missing
        // files, which kills the fight screen. Skipping the load lets the fight
        // run without UI sprites (lifebars will be invisible, but the fight works).
        ...
        if (file != "NO_FILE" && isFile(file)) {
                gFightUIData.mFightSprites = loadMugenSpriteFileWithoutPalette(file);
        } else {
                logWarning("[FIGHTUI] No fight.sff specified — skipping fight sprites.");
        }
        ...
        if (file != "NO_FILE" && isFile(file)) {
                ...
                logWarning("[FIGHTUI] No fightfx.sff specified — skipping fight FX sprites.");
        }
        ...
        if (file != "NO_FILE" && isFile(file)) {
                ...
                logWarning("[FIGHTUI] No fightfx.air specified — skipping fight FX animations.");
        }
        ...
        if (file != "NO_FILE" && isFile(file)) {
                ...
                logWarning("[FIGHTUI] No snd specified — skipping fight sounds.");
        }
        ...
        if (file != "NO_FILE" && isFile(file)) {
                ...
                logWarning("[FIGHTUI] No common.snd specified — skipping common sounds.");
        }
}
